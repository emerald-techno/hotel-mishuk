...
function getReceiveInfo() {
    const receiveId = $("#Id").val();

    if (receiveId > 0) {
        const url = API + "Receive/GetById/" + receiveId;

        $.get(url, function (rData) {
            if (rData !== undefined) {
                console.log("receive-info", rData);

                if (rData.tranDtls.length > 0) {
...
$(document.body).on("click", "#UpdateReceiveSubmitBtn", function () {
    console.log('ItemList', itemList)
    if (itemList.length > 0) {
        swal({
            title: "Update Confirmation",
            text: "Are you sure you want to update the receive?",
            icon: "warning",
            buttons: {
                cancel: {
                    text: "Cancel",
            ...
            if (willSubmit) {
                ...
                if (isValid) {
                    ...
                    startFormPosting("#EditDirectReceiveForm");
                } else {
                    failedMsg(msg);
                }
            } else {
...
function renderTBody() {
    ...
    if (itemList.length > 0) {
        ...
        itemList.forEach((v, i) => {
            ...
            const hiddenCell = `<input type='hidden' name='TranDtls[${i}].Id' value='${v.id}'/>
                                <input type='hidden' name='TranDtls[${i}].CategoryId' value='${v.categoryId}'/>
                                <input type='hidden' name='TranDtls[${i}].LedgerId' value='${v.ledgerId}'/>`;

            const itemCell = `<td>${hiddenCell}<input type='hidden' name='TranDtls[${i}].ItemId' value=${v.itemId} />${v.itemName}</td>`;
