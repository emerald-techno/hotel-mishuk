...
$(document.body).on("click", "#ShowReportBtn", function () {
    ...
    const url = API + "AccTranMst/ReceiptAndPayments";
...
$(document.body).on("click", "#ReportPrintBtn", function () {
    ...
    if (!hasAnyError(model.StrFromDate) && !hasAnyError(model.StrToDate)) {
        const url = `${API}AccTranMst/ReceiptAndPaymentsPrint?fromDate=${model.StrFromDate}&toDate=${model.StrToDate}&mishukLedgerId=${model.MishukLedgerId}`;
