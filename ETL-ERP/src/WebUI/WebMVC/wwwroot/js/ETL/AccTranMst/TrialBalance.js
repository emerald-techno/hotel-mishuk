...
//    const url = API + "AccTranMst/TrialBalance";
...
//        const url = `${API}AccTranMst/TrialBalancePrint?fromDate=${model.StrFromDate}&toDate=${model.StrToDate}`;
...
$(document.body).on("click", "#TrialBalancePrintBtn", function () {
    ...
    if (!hasAnyError(model.StrFromDate) && !hasAnyError(model.StrToDate) && model.FinYearId > 0) {
        const url = `${API}AccTranMst/TrialBalancePrint?fromDate=${model.StrFromDate}&toDate=${model.StrToDate}&finYearId=${model.FinYearId}&accountId=${model.MishukLedgerId}`;
        window.open(url, "_blank");
    }
});
