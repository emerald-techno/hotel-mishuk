...
$(document.body).on("change", "#EmployeeId", function () {
    ...
    if (year > 0 && month > 0 && employeeId > 0) {
        getExistingData(year, month, employeeId);
    } else {
        shiftList = [];
        createShiftTable();
    }
...
function getExistingData(year, month, employeeId) {
    if (year > 0 && month > 0) {
        shiftList = [];

        const url = `${API}ShiftManagement/GetExistShift?year=${year}&month=${month}&employeeId=${employeeId}`;
...
$(document.body).on("click", "#EmpSetupSubmitBtn", function () {
    ...
    if (shiftList.length > 0) {
        ...
        if (isValid) {
            startFormPosting("#EmployeeShiftSetupForm");
        } else {
            failedMsg(msg);
        }
    } else {
